...
//命名非法字符
static BOOL ZZCharIsIllegal(char c) {
    return !((c >= '0' && c <= '9') || (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || c == '_');
}

//过滤非法字符 (用'_')
static NSString *ZZFilterIllegalChar(NSString *string) {
    NSMutableString *res = [NSMutableString string];
    for(int i = 0; i < string.length; ++i){
        unichar c = [string characterAtIndex:i];
        [res appendString:ZZCharIsIllegal(c) ? @"_" : [NSString stringWithFormat:@"%c", c]];
    }
    return res;
}

//指定大写范围
static NSString *ZZSpecifyCapitalizeWithString(NSString *string, NSUInteger length) {
...
//全大写
static NSString *ZZCapitalizeWithString(NSString *string) {
    if (!string || string.length == 0) return string;
    return [string capitalizedString];
}

//驼峰命名
static NSString *ZZCamelCaseWithString(NSString *string) {
    if (!string || string.length == 0) return string;
    ...
    while (true) {
        ...
        if (scan && tmp) {
            [res appendString:ZZSpecifyCapitalizeWithString(tmp, 1)];
        }
...
//属性名保留字
static NSSet<NSString *> *ZZPropertyReservedWord(void) {
...
//属性名非法字符前缀
static NSSet<NSString *> *ZZPropertyIllegalCharPrefix(void) {
    static NSSet *set = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        set = [NSSet setWithObjects:
...
//属性名非法数字前缀
static NSSet<NSString *> *ZZPropertyIllegalNumberPrefix(void) {
    static NSSet *set = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        set = [NSSet setWithObjects:
...
static NSSet<NSString *> *ZZClassVarsExceptPrefix(Class cls) {
...
@interface ZZModelFileNameHandle ()

@property (nonatomic, strong) NSMutableSet *classNames;
@end

@implementation ZZModelFileNameHandle
...
- (NSString *)ybmf_classNameWithPrefix:(NSString *)prefix suffix:(NSString *)suffix key:(id)key {
    ...
    //过滤关键字
    keyStr = ZZFilterIllegalChar(keyStr);
    
    //驼峰命名
    keyStr = ZZCamelCaseWithString(keyStr);
...
- (NSString *)ybmf_propertyNameWithKey:(id)key existKeys:(nonnull NSArray *)existKeys {
    ...
    //如果是保留字，全大写
    if ([ZZPropertyReservedWord() containsObject:keyStr]) {
        keyStr = ZZCapitalizeWithString(keyStr);
    }
    
    //前缀有保留字，保留字部分大写
    [ZZPropertyIllegalCharPrefix() enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, BOOL * _Nonnull stop) {
        if ([keyStr hasPrefix:obj]) {
            keyStr = ZZSpecifyCapitalizeWithString(keyStr, obj.length);
            *stop = YES;
        }
    }];
    
    //前缀是数字，拼接一个下划线
    if ([ZZPropertyIllegalNumberPrefix() containsObject:[keyStr substringToIndex:1]]) {
    ...
    NSUInteger suf = 0;
    NSSet *illegalVars = ZZClassVarsExceptPrefix([ZZModelCreatorConfig shareConfig].baseClass);
